...
exports.standardDeviation = dataset => {
  let sum = 0,
    data = dataset,
    scores = new Array;
  // let average = (stats.avgAutoInner * 6) + (stats.avgAutoLow * 2) + (stats.avgAutoOuter * 4) + (stats.avgTeleLow) + (stats.avgCrossLine * 5) + (stats.avgPark * 5) + (stats.avgBuddyHang * 50) + (stats.avgTeleOuter * 2) + (stats.avgTeleInner * 3) + (stats.avgHang * 25)
  data.forEach(e => {
    sum = 0;
    for (let i in e) {
      switch (i) {
